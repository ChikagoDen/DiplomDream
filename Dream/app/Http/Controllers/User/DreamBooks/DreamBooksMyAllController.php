...
class DreamBooksMyAllController extends Controller
{
    public function index(){
        ...
        if (isset($_GET['idComent'])) {
            ...
            if (is_numeric($idComent)) {
               comment_table::where('idcomment_table', $idComent)->increment("comment_like");
        ...
        if (isset($_GET['comentId'])) {
            ...
            if (is_numeric($idComent)) {
                comment_table::where('idcomment_table', $idComent)->decrement("comment_like");
                // DB::table('comment_table')
                //     ->where('idcomment_table', '=', $idComent)
        ...
        // все по коментам
        $dataALLComent=comment_table::with(['dream', 'userCommit'])
        ...
        // все сны
        $AllDream=dream_user_table::with('user')
    ...
    public function coment(){
        ...
        elseif (number_format(request()->input('commentLevel'))==2) {
            ...
            $comment_table=new comment_table;
            $comment_table->comment_id_dream=number_format(request()->input('dreamName'));
            $comment_table->comment_id_user=number_format(request()->input('userName'));
            $comment_table->comment_discription=request()->input('descriptionDreamComent');
            $comment_table->comment_nesting_level=2;
            $comment_table->comment_id_user_answer=number_format(request()->input('commentId'));            
            $comment_table->save();
            // DB::table('dream_book_biblioteca.comment_table')
            //     ->insert([
